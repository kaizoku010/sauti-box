...
const CopyrightPage = () => {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validFiles = selectedFiles.filter(file => {
      const isValidType = ['audio/mpeg', 'audio/wav', 'audio/flac'].includes(file.type);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // Add metadata
      formData.append('title', ((event.currentTarget as HTMLFormElement)
      .elements.namedItem('title') as HTMLInputElement)?.value || '');
      formData.append('artist', ((event.currentTarget as HTMLFormElement)
      .elements.namedItem('artist') as HTMLInputElement)?.value || '');
      formData.append('genre', ((event.currentTarget as HTMLFormElement)
      .elements.namedItem('genre') as HTMLSelectElement)?.value || '');
      ...
      // Reset form
      setFiles([]);
      (event.currentTarget as HTMLFormElement).reset();      alert('Upload successful!');
    } catch (error) {
