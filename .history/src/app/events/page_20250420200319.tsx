...
import { FiCalendar, FiMapPin, FiClock, FiTicket, FiShare2, FiHeart, FiFilter } from 'react-icons/fi';
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
          ...
          <div className="mt-4 md:mt-0">
            <div className="flex space-x-2">
              <button
                className={`px-4 py-2 rounded-md ${filter === 'all' ? 'border-2 border-primary text-primary' : 'border-2 border-gray-700 text-gray-300 hover:border-primary hover:text-primary'}`}
                onClick={() => setFilter('all')}
              >
                All Events
              </button>
              <button
                className={`px-4 py-2 rounded-md ${filter === 'upcoming' ? 'border-2 border-primary text-primary' : 'border-2 border-gray-700 text-gray-300 hover:border-primary hover:text-primary'}`}
                onClick={() => setFilter('upcoming')}
              >
                Upcoming
              </button>
              <button
                className={`px-4 py-2 rounded-md ${filter === 'saved' ? 'border-2 border-primary text-primary' : 'border-2 border-gray-700 text-gray-300 hover:border-primary hover:text-primary'}`}
            ...
            {filteredEvents.map((event) => (
              <div key={event.id} className="card group overflow-hidden">
                <div className="relative aspect-video overflow-hidden">
                  ...
                  />
                  <div className="absolute top-0 left-0 bg-primary text-white px-3 py-1 m-3 rounded-md">
                  ...
                  <div className="flex items-center justify-between mt-4">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <Link
                        ...
                        className="flex items-center px-3 py-1 border-2 border-primary text-primary rounded-md hover:bg-primary/10"
                      >
                        <FiTicket className="mr-1" size={16} />
                        <span>Tickets</span>
                      </Link>
                    </div>
                  </div>
            ...
            <div className="md:flex">
              ...
              <div className="p-6 md:w-1/2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Link
                    href="#"
                    className="px-4 py-2 border-2 border-primary text-primary rounded-md hover:bg-primary/10 flex items-center"
                  >
                    <FiTicket className="mr-2" />
                    Get Tickets
                  </Link>
                </div>
              </div>
