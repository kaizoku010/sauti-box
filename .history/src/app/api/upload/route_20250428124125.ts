...
export async function POST(request: Request) {
  try {
    ...
    console.log('Token received in upload API:', token);

    const payload = verifyToken(token);

    if (!payload) {
      console.error('Invalid token in upload API');
      return NextResponse.json(
        { error: 'Invalid token' },
        { status: 401 }
      );
    ...
    if (payload.type !== 'artist') {
      console.error('Non-artist attempting to upload music');
      return NextResponse.json(
    ...
    console.log('Authentication successful for artist:', payload.id);
    ...
    const artistId = formData.get('artistId') as string || payload.id;
