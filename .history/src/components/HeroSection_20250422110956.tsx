      ...
      <div className="relative h-full max-w-6xl mx-auto px-6 flex items-center pt-24">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <div className={`transition-all duration-500 ${isTransitioning ? 'opacity-0 -translate-x-10' : 'opacity-100 translate-x-0'}`}>
            <div style={{color:"#ffffffad !important"}} className="text-sm font-weight-800 font-heavy text-primary mb-2">Featured Album</div>
            ...
            <div className="flex space-x-4">
              ...
              <button 
              id="btn-text-mobile more-info"
              className="px-6 py-3 bg-gray-800/50 border border-gray-700 text-white rounded-full flex items-center hover:bg-gray-700/50 transition-colors">
          ...
          <div className="hidden md:block">
            ...
            >
              ...
              <div className="absolute -bottom-4 -right-4 bg-black/80 backdrop-blur-md p-4 rounded-lg shadow-lg z-20 space-y-2">
                {/* <div>
                  <p className="text-xs text-gray-400">Upload Year</p>
                  <p id='desc' className="text-white font-bold">{currentAlbum.releaseDate}</p>
                </div> */}
                <div>
                  <p  className="text-xs text-gray-400">Purchases</p>
                  <p id='desc' className="text-white font-bold">{currentAlbum.stats.purchases.toLocaleString()}</p>
                </div>
                <div>
                  <p  className="text-xs text-gray-400">Streams</p>
                  <p id='desc' className="text-white font-bold">{currentAlbum.stats.streams.toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
