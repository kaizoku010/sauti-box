...
const MusicPlayer: React.FC = () => {
  ...
  const recordStream = async (song: any) => {
    if (!song || (!song._id && !song.id)) return;
    ...
    try {
      ...
      console.log('Stream recorded for song:', songId);
    } catch (error) {
      console.error('Error recording stream:', error);
    }
  };

  // Track if we've already recorded a stream for this song
  const [streamRecorded, setStreamRecorded] = useState(false);

  useEffect(() => {
    // Initialize or update audio element when currentSong changes
    if (currentSong) {
      const audioUrl = currentSong.audioUrl || currentSong.cloudinaryUrl;
      ...
      if (audioUrl) {
        ...
        // Create new audio element with the current song
        ...
        // Reset stream recorded flag when song changes
  ...
  // Record stream when a song has played for at least 30 seconds
  useEffect(() => {
    if (currentSong && isPlaying && currentTime >= 30 && !streamRecorded) {
      recordStream(currentSong);
      setStreamRecorded(true);
    }
  }, [currentSong, isPlaying, currentTime, streamRecorded]);
          ...
          <div className="flex items-center w-1/4">
            <div
              ...
              className="w-16 h-16 rounded-2xl overflow-hidden
               mr-4 flex-shrink-0 shadow-md album-art-animation">
              <img
                src={currentSong.coverImageUrl || currentSong.coverImage || Vinka.src}
                alt={currentSong.title}
                className="w-full h-full object-cover"
              />
            </div>
