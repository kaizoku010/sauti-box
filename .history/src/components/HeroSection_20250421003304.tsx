            ...
            >
              ...
              <div className="absolute top-4 right-4 flex flex-col gap-2 z-20">
                ...
                <div className="bg-primary/20 backdrop-blur-md px-4 py-2 rounded-full border border-primary/30 flex items-center gap-2">
                  ...
                  <span className="text-primary font-medium">{currentAlbum.releaseDate}</span>
                </div>

                {/* Purchases Badge */}
                <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex items-center gap-3 group hover:bg-black/60 transition-all">
                  <div className="flex flex-col">
                    <span className="text-xs text-gray-400">Purchases</span>
                    <span className="text-white font-bold">{currentAlbum.stats.purchases.toLocaleString()}</span>
                  </div>
                ...
                <div className="bg-black/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex items-center gap-3 group hover:bg-black/60 transition-all">
                  <div className="flex flex-col">
                    ...
                    <span className="text-white font-bold">{currentAlbum.stats.streams.toLocaleString()}</span>
                  </div>
                  <div className="h-8 w-[1px] bg-white/10"></div>
                  <div className="w-2 h-2 rounded-full bg-blue-400 group-hover:animate-ping"></div>
                </div>
              </div>

              {/* Gradient Overlay */}
              <div className="absolute inset-0 rounded-lg bg-gradient-to-tr from-black/20 via-transparent to-transparent"></div>
            </div>
          </div>
        </div>
