          ...
          <div className="hidden md:block">
            ...
            >
              <img
                ...
                id='hero-album-art'
                className="relative w-full h-auto rounded-lg shadow-2xl transition-transform duration-500 object-cover"
                style={{ objectFit: 'cover' }}
              />

              {/* Stats Overlay */}
              <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 to-transparent rounded-b-lg z-20">
                <div className="flex items-center justify-between text-white">
                  {/* Release Year */}
                  <div className="flex items-center space-x-2">
                    <span className="text-primary text-lg font-bold">{currentAlbum.releaseDate}</span>
                  </div>
                  ...
                  <div className="flex items-center space-x-6">
                    ...
                    <div className="text-right">
                      <div className="text-lg font-bold">{currentAlbum.stats.purchases.toLocaleString()}</div>
                      <div className="text-xs text-gray-400 uppercase tracking-wider">Purchases</div>
                    </div>

                    {/* Streams */}
                    <div className="text-right">
                      <div className="text-lg font-bold">{currentAlbum.stats.streams.toLocaleString()}</div>
                      <div className="text-xs text-gray-400 uppercase tracking-wider">Streams</div>
                    </div>
                  </div>
                </div>
              </div>
