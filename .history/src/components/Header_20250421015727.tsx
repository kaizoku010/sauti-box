...
import { FiSearch, FiBell, FiUser, FiShoppingCart, FiLogIn, FiUserPlus, FiMusic, FiX } from 'react-icons/fi';
...
const Header = () => {
  ...
  return (
    <header className="flex items-center justify-between px-6 sm:px-8 md:px-10 py-3 glass-effect text-white rounded-[30px] w-full backdrop-blur-xl">
      <div className="flex items-center">
        <Link href="/" className="flex items-center">
          <img src={LogoFull.src} alt="SautiBox" className="h-9 object-contain" />
        </Link>
      </div>
      ...
      <div className="flex items-center space-x-5">
        ...
        {isLoggedIn ? (
          ...
          <div className="flex items-center space-x-2">
            <Link href="/login">
              <button id='curved' className="px-3 py-1.5 border border-primary-89 text-white bg-transparent hover:bg-primary-89 hover:text-black rounded-full text-sm font-medium flex items-center backdrop-blur-sm transition-colors">
            ...
            <div className="dropdown dropdown-end group">
              <button id='curved' className="px-3 py-1.5 border border-primary-89 text-white bg-transparent hover:bg-primary-89/10 hover:text-white rounded-full text-sm font-medium flex items-center backdrop-blur-sm">
              ...
              <div className="dropdown-content hidden group-hover:block absolute right-0 mt-2 w-48 glass-effect rounded-[20px] shadow-lg z-50 overflow-hidden">
                <Link href="/register" className="block px-4 py-2 hover:bg-primary/10 hover:text-primary-89 transition-colors">
                  <FiUser className="inline mr-2" size={16} />
                  User Sign Up
                </Link>
                <Link href="/artist/register" className="block px-4 py-2 hover:bg-primary/10 hover:text-primary-89 transition-colors">
