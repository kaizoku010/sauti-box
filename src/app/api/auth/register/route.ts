...
export async function POST(request: Request) {
  try {
    ...
    const { name, email, password, userType, profileImage, coverImage } = body;
    ...
    if (userType === 'artist' && (!profileImage || !coverImage)) {
      return NextResponse.json(
        { error: 'Profile and cover images are required for artists' },
        { status: 400 }
      );
    }
